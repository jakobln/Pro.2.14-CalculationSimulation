...
namespace CalculationSimulation
{
    ...
    public class Simulator
    {
        ...
        {
            int result = Cache.CacheSettings.CalculateNoValue;

            if (x < Cache.CacheSettings.DimensionX && x >= 0 && y < Cache.CacheSettings.DimensionY && y >= 0)
            {
                if (_useCache)
                {
                ...
                if (result == Cache.CacheSettings.CalculateNoValue)
                {
